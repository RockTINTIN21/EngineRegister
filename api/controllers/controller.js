...
router.patch('/updateEngine/:id', upload.single('file'), async (req, res) => {
    try {
        ...
        await engineInstance.updateEngine(
            ...
            req.body.status,
            req.body.comments,
            imageFilePath === 'null' ? null : imageFilePath,
...
router.get('/getAllEngines', async (req, res) => {
    try {
        const engineInstance = new Engine();
        const engines = await engineInstance.getAllEngines();
...
router.get('/getPositions', async (req, res) => {
    try {
        const engineInstance = new Engine();
        const positions = await engineInstance.getPositions();
        res.status(200).send({
            status: 'success',
            data: positions
        });
...
router.get('/getEngineByID', async (req, res) => {
    try {
        ...
        console.log('Received Engine ID:', engineId);
        const engineInstance = new Engine();
